...
class MonthdataController extends Controller {
    async index(){
        await this.ctx.render('monthdata.html', { "title": "test", "list": [1, 2] })
    }
    ...
    async save(){
        ...
        for(let i in monthdata){
            ...
            if(lList[name] && lList[name][month]){
                monthdata[i]["teamtime"] = this.getTeamtime(userMap, lList[name][month])
            }
            else{
                monthdata[i]["teamtime"] = 0
            }
    ...
    async find() {
        let conditions = this.ctx.request.query
        let result = "" 
        try{
    ...
    async monthdata(){
        const MonthdataService = this.ctx.service.monthdata

        let {months} = this.ctx.request.query
        let result = await MonthdataService.Find({month:{$in:months.split(",")}})
