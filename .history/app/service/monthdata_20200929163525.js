...
class MonthdataService extends Service {
    ...
    async GetMonthdata(months = "") {
        ...
        for(let i in monthdata){
            let {name, hstime, bztime, cehstime, teamtime} = monthdata[i]
            if(!mdMap[name]){
                mdMap[name] = { name, hstime, bztime, cehstime, teamtime }
                mdMap[name]["cehstimetotal"] = (parseFloat(cehstime) + parseFloat(teamtime)).toFixed(2)
            }
            else{
                ...
                mdMap[name]["teamtime"] = (parseFloat(mdMap[name]["teamtime"]) + parseFloat(teamtime)).toFixed(2)
