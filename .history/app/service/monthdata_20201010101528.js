...
class MonthdataService extends Service {
    ...
    async GetMonthdata(months = "") {
        ...
        for(let i in monthdata){
            let {name, dept, hstime, bztime, cehstime, teamtime, month} = monthdata[i]
            if(!mdMap[name]){
            ...
            mdMap[name]["cehstime"+month] = (parseFloat(cehstime)).toFixed(2)
            mdMap[name]["teammonth"+month] = parseFloat(teamtime) > 0 ? teamtime : 0
        }
